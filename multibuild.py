...
Since Cloakfox is NOT meant to be used as a daily driver, no installers are provided.
"""
...
class BSYS:
    ...
    def build(self):
        """Build the Cloakfox source code"""
        os.environ['BUILD_TARGET'] = f'{self.target},{self.arch}'
        run('make build')

    def package(self):
        """Package the Cloakfox source code"""
    ...
    def assets(self) -> List[str]:
        ...
        package_pattern = f'cloakfox-*-{self.target[:3]}.{self.arch}.zip'
        return glob.glob(package_pattern)

    @staticmethod
    def clean():
        """Clean the Cloakfox directory"""
        run('make clean')
...
def main():
    parser = argparse.ArgumentParser(description="Easy build CLI for Cloakfox")
    parser.add_argument(
        "--target",
