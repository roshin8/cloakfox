...
"""
The script that patches the Firefox source into the Cloakfox source.
...
@dataclass
class Patcher:
    """Patch and prepare the Cloakfox source"""

    moz_target: str
    target: str

    def cloakfox_patches(self):
        """
        Apply all patches
        """
...
def extract_args():
    ...
    if len(args) != 2:
        sys.stderr.write('error: please specify version and release of cloakfox source')
        sys.exit(1)
