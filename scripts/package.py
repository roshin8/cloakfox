...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move Cloakfox/Cloakfox.app -> Cloakfox.app
            nightly_dir = os.path.join(temp_dir, 'Cloakfox')
            shutil.move(
                os.path.join(nightly_dir, 'Cloakfox.app'), os.path.join(temp_dir, 'Cloakfox.app')
            )
            # Remove old app dir and all content in it
            shutil.rmtree(nightly_dir)
        else:
            # Move contents out of cloakfox folder if it exists
            old_cloakfox_dir = os.path.join(temp_dir, 'cloakfox')
            cloakfox_dir = os.path.join(temp_dir, 'cloakfox-folder')
            if os.path.exists(old_cloakfox_dir):
                # Rename cloakfox_dir
                os.rename(old_cloakfox_dir, cloakfox_dir)
                for item in os.listdir(cloakfox_dir):
                    shutil.move(os.path.join(cloakfox_dir, item), temp_dir)
                os.rmdir(cloakfox_dir)

        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'Cloakfox.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Package Cloakfox for different operating systems.'
    )
    ...
    parser.add_argument('--version', required=True, help='Cloakfox version')
    parser.add_argument('--release', required=True, help='Cloakfox release number')
    parser.add_argument(
...
def main():
    ...
    with temp_cd(src_dir):
        ...
        search_path = os.path.abspath(
            f'obj-{moz_target}/dist/cloakfox-{args.version}-{args.release}.*.{file_ext}'
        )
    ...
    package_pattern = f'cloakfox-{args.version}-{args.release}.en-US.*.{file_ext}'
