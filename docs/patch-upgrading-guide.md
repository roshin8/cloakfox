...
This guide provides step-by-step instructions for updating Cloakfox patches when upgrading Firefox versions. Patches frequently break due to Firefox API changes, file reorganizations, and line number shifts.
...
All Cloakfox patches are in the `patches/` directory:
...
grep -r "FunctionName" cloakfox-<version>/ --include="*.cpp"
```
