...
Cloakfox spoofs fingerprints globally via `CLOAK_CONFIG` — every browser context shares the same identity. These patches add **per-context isolation**, so each Playwright context can have a unique, deterministic fingerprint. This lets you run multiple concurrent sessions from a single Cloakfox process without cross-context correlation.
...
const browser = await firefox.launch({
  executablePath: '/path/to/cloakfox',
});
...
**Why `addInitScript`?** Each `window.setXxx()` function self-destructs after the first call. On new tabs or navigations, Cloakfox recreates the functions, and the init script calls them again before any page scripts can run. This ensures fingerprints are always applied and functions are never visible to website code.

**Why `typeof` guards?** The init script runs on every page load. If Cloakfox is ever used without these patches (e.g. vanilla build), the guards prevent `ReferenceError`. They also handle the case where the function was already called and self-destructed.
...
### Configuration (`settings/cloakfox.cfg`)

The `cloakfox.cfg` file sets Firefox preferences at startup (before `prefs.js` is loaded). Key settings:
...
**MaskConfig fallback:** If no per-context seed is set, checks `MaskConfig::GetUint32("audio:seed")` from `CLOAK_CONFIG`. This enables the Cloakfox Python package to set a global audio seed without per-context JavaScript.
...
**How it works:** Stores dimensions per context, then hooks `nsScreen::GetRect()` with a three-tier fallback: per-context values -> global `CLOAK_CONFIG` -> vanilla Firefox.
...
This replaces the old `screen-hijacker.patch` (which only supported global config). It includes the same global `CLOAK_CONFIG` fallback, so it works for both single-context and multi-context use cases.
...
**Controls:** `navigator.platform`, `navigator.oscpu`, `navigator.hardwareConcurrency`, `navigator.userAgent`, and `navigator.appVersion` — per-context with global `CLOAK_CONFIG` fallback.

**How it works:** Stores values per context via `NavigatorManager`, then hooks these Navigator methods with a three-tier fallback: per-context values -> global `CLOAK_CONFIG` -> vanilla Firefox:
...
**`setNavigatorUserAgent`:** Stores a per-context User-Agent string so workers report the correct UA. Without this, workers on Linux would read the global `CLOAK_CONFIG` UA (which may be a Linux UA) even when the per-context fingerprint specifies macOS. The WorkerNavigator hook checks `NavigatorManager` BEFORE `MaskConfig`, ensuring workers match the main page.
...
**Lazy timezone init:** Also adds `EnsureGlobalTimezoneInitialized()` — a lazy initializer that reads `timezone` from `CLOAK_CONFIG` on first access to `GetPlatform()` or `HardwareConcurrency()`. This replaced a static initializer that caused SIGSEGV crashes because SpiderMonkey wasn't ready at init time.
...
**How it works:** Stores vendor and renderer strings per context via `WebGLParamsManager`, then hooks `ClientWebGLContext::GetParameter()` to intercept `WEBGL_debug_renderer_info` queries. Three-tier fallback: per-context values -> global `CLOAK_CONFIG` -> real hardware values.
...
**Global MaskConfig scope:** Beyond per-context vendor/renderer, this patch also includes comprehensive global `CLOAK_CONFIG` spoofing for many other WebGL parameters — context attributes, shader precision, extensions, enabled states, and array parameters. These global overrides apply to all contexts equally and don't require per-context JavaScript.
...
**MaskConfig fallback:** If no per-context seed is set, checks `MaskConfig::GetUint32("canvas:seed")` from `CLOAK_CONFIG`.
...
These patches read from `CLOAK_CONFIG` at startup and apply to all contexts equally. They don't expose any `window.setXxx()` functions.
...
**`FONTCONFIG_PATH` environment variable:** Must be set when launching Cloakfox on Linux. Points to the correct OS-specific fontconfig directory (e.g. `cloakfox/fontconfig/macos/`). The Go launcher sets this dynamically based on the target OS.

---

## Python Library Changes

The Cloakfox Python package (`pythonlib/`) generates fingerprints for both `NewBrowser` (global CLOAK_CONFIG) and `NewContext` (per-context init script). **BrowserForge is the default for both paths.** Real fingerprint presets are available as an opt-in alternative.
...
|----------|--------|-------|
| UA, platform, HWC, oscpu | BrowserForge or preset | UA version patched to match Cloakfox Firefox version |
...
- `from_preset()` — converts real preset to CLOAK_CONFIG format
- `from_browserforge()` — converts BrowserForge Fingerprint to CLOAK_CONFIG using `browserforge.yml` mappings
...
- `launch_options()` — builds CLOAK_CONFIG env var, Playwright args, and Firefox prefs
...
**`properties.json`** — Includes `audio:seed` and `canvas:seed` as `CLOAK_CONFIG` properties (uint type). These enable the MaskConfig fallback in the audio and canvas patches when using global config without per-context JavaScript.

**`cloakfox.cfg`** — Sets `fission.autostart=true` and `dom.ipc.processPrelaunch.enabled=false`. No `dom.ipc.processCount` override needed with cross-process storage.
