...
inline const nlohmann::json& GetJson() {
  ...
  std::call_once(initFlag, []() {
    ...
    while (true) {
      std::string envName = "CLOAK_CONFIG_" + std::to_string(index);
    ...
    if (jsonString.empty()) {
      // Check for the original CLOAK_CONFIG as fallback
      auto originalConfig = get_env_utf8("CLOAK_CONFIG");
    ...
    if (!nlohmann::json::accept(jsonString)) {
      printf_stderr("ERROR: Invalid JSON passed to CLOAK_CONFIG!\n");
      jsonConfig = nlohmann::json{};
      return;
    }
